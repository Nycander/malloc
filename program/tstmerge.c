...
int main(int argc, char * argv[])
{
  ...
  fprintf(stderr, "-- Testing merging of deallocated large blocks ( >= %u bytes)\n", (unsigned)largeSize);

  p1 = (char *)malloc(largeSize);
  p2 = (char *)malloc(largeSize);

  if(p1 == NULL || p2 == NULL) fprintf(stderr, "* ERROR: unable to allocate memory of size %u bytes\n", (unsigned) largeSize);
  ...
  if(p1 == NULL) fprintf(stderr, "* ERROR: unable to allocate memory of size %u bytes\n", (unsigned)largeSize);

  newBrk2 = (char *)sbrk(0);

  if(((double)(newBrk2 - oldBrk))/((double)(newBrk1 - oldBrk)) > 1)
    fprintf(stderr, "* ERROR: not good enough usage of memory (probably incorrect merging of deallocated memory blocks)\nYour implementation used %u bytes more than expected\n", ((unsigned)(newBrk2-newBrk1)));
  else
